...
def test_search_returns_hits_with_expected_shape(
...
def test_search_unknown_source_in_filter_is_ignored(
    store: Store, tmp_path: Path
) -> None:
    _seed(store, tmp_path, "vault", {"a.md": "# A\n\nbody\n"})
    tools = build_tools(store, FakeEmbedder(), configured_sources=["vault"])
...
def test_search_all_unknown_sources_returns_empty(
    store: Store, tmp_path: Path
) -> None:
    _seed(store, tmp_path, "vault", {"a.md": "# A\n\nbody\n"})
    tools = build_tools(store, FakeEmbedder(), configured_sources=["vault"])
...
def test_search_empty_sources_list_returns_empty(
    store: Store, tmp_path: Path
) -> None:
